...
REMPL = [
    ...
    # 2. Lumiere ambiante nommee (viewer uniquement, 0.45)
    ('scene.add(new THREE.AmbientLight(0xffffff, 0.45));',
     'const lumAmb = new THREE.AmbientLight(0xffffff, 0.45);\n    scene.add(lumAmb);'),
    # 3. Bloc ambiance apres le soleil
    ('sun.shadow.bias = -0.0005;\n    scene.add(sun);', BLOC_AMBIANCE),
    # 4. Animation de la pluie dans la boucle
    ('      renderer.render(scene, camera);', BLOC_PLUIE_ANIM),
    # 5. Selecteur dans la rangee de boutons
    ('                      {m.label}\n                    </button>\n                  ))}', BLOC_SELECT),
    # 6. Transmission au viewer
    ...
    # 7. Reconstruction quand le fond change
...
for a, b in REMPL:
    n = txt.count(a)
    if n != 1:
        print("ABANDON : ancre en", n, "exemplaire(s), attendu 1 :", a[:70].replace("\n", " / "))
        ok = False
if ok == False:
    raise SystemExit(1)
