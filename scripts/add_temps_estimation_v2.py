...
# -*- coding: utf-8 -*-
"""
DEVIA - Modif 6 v2 : Estimations temps + correction "Genere" -> "Genere" (sans accent)
On corrige aussi un accent mal place (Genere -> Genere) car c'etait un imperatif,
pas un participe passe.
"""
...
backup_name = f"devia.jsx.backup_{datetime.now().strftime('%Y%m%d_%H%M%S')}_before_temps_v2"
...
# ================================================================
# MOD 0 : Corriger "Genere" mal accentue dans le prompt
# (c'etait un imperatif, on remet sans accent pour eviter ambiguite)
# ================================================================

# On corrige "Généré 12 a 18 postes" -> "Genere 12 a 18 postes" (imperatif)
# Note : on garde le mot SANS accent dans le prompt, car c'est destine a l'IA
# (et c'etait l'orthographe d'origine dans la code)

corrections_prompt = [
    ('Généré 12 a 18 postes', 'Genere 12 a 18 postes'),
    ('Généré entre 3 et 8 postes', 'Genere entre 3 et 8 postes'),
]

for old, new in corrections_prompt:
    if old in content:
        nb = content.count(old)
        content = content.replace(old, new)
        print(f"[OK] Correction prompt : '{old}' -> '{new}' ({nb}x)")
        modifs += nb

# ================================================================
# MOD 1 : Modifier le prompt IA - approche plus robuste
# On cherche maintenant la nouvelle phrase corrigee
# ================================================================
...
if "temps_fabrication_h" in content:
    print("[INFO] Prompt deja modifie pour les temps")
    ...
    print("[ERREUR] Fin du prompt non trouvee meme apres correction")
    print("[INFO] On affiche le contexte autour de '12 a 18 postes' pour debug :")
    idx = content.find("12 a 18 postes")
    if idx >= 0:
...
# ================================================================
# MOD 3 : Ajouter la CARD "Estimation temps" avant FeuilleCalcTable
# ================================================================
