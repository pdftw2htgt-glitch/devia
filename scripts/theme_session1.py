...
# -*- coding: utf-8 -*-
"""
DEVIA - Mode Clair/Sombre Session 1
- Objet theme avec palettes dark + light
- State themeMode + persistance localStorage
- cardStyle/inputStyle/btnPrimary/btnSecondary dynamiques
- Fond + header + nav adaptatifs
...
backup_name = f"devia.jsx.backup_{datetime.now().strftime('%Y%m%d_%H%M%S')}_before_theme_session1"
...
# On l'ajoute dans DeviaMain, apres "const [usageLogs, setUsageLogs] = useState([]);"
...
if "const [themeMode, setThemeMode]" in content:
    ...
    print("[OK] State themeMode + persistance ajoutes")
    modifs += 1
else:
...
# ================================================================
# MOD 2 : Definir l'objet theme avec 2 palettes
# On l'ajoute juste avant cardStyle
# ================================================================

old_cardstyle_marker = 'const cardStyle = {'

new_with_theme = '''// ====== Palettes Theme (Mode Clair / Sombre) ======
const themes = {
  dark: {
    // Fond global
  ...
  light: {
    // Fond global (blanc pur avec touche tres subtile)
    ...
    textMuted: "#8a8d9c",
    textFaint: "#a8abb8",
    // Accents (or plus fonce pour lisibilite sur fond clair)
    gold: "#b8860b",
    goldDark: "#9c7000",
  }
};
...
if "// ====== Palettes Theme" in content:
    print("[INFO] Objet themes deja defini")
elif old_cardstyle_marker in content:
    content = content.replace(old_cardstyle_marker, new_with_theme, 1)
    print("[OK] Palettes theme (dark + light) ajoutees")
    modifs += 1
else:
    print("[ERREUR] cardStyle non trouve")
    sys.exit(1)

# ================================================================
# MOD 3 : Rendre cardStyle/inputStyle/btnPrimary/btnSecondary
# dynamiques en fonction du theme actuel
# ================================================================

# Strategie : on remplace les const par des const qui referencent un objet `t`
# qui est calcule depuis themeMode (recupere via const t = themes[themeMode] || themes.dark)
# Mais ces const sont declarees AU NIVEAU DU MODULE (hors composant)
# Donc on doit les declarer DANS le composant DeviaMain
# OU les transformer en fonctions

# Approche : transformer cardStyle (const) en une fonction qui prend le theme en parametre
# OU plus simple : redefinir cardStyle dans DeviaMain via useMemo

# Approche simple : on garde le cardStyle hors composant, mais on ajoute une variable
# Mais il faut acceder a themeMode pour switcher

# Decision : on ajoute DANS DeviaMain une serie de styles dynamiques
# qui surchargent les const globales

# Plus simple encore : on remplace les references aux 4 styles globaux par
# les nouveaux styles dynamiques dans le scope de DeviaMain

# Methode propre : ajouter dans DeviaMain (apres themeMode) :
# const t = themes[themeMode] || themes.dark;
# const cardStyle = { background: t.cardBg, border: ..., ... }
# ... etc

# Ca va shadow les const globales = parfait

# Trouver l'endroit dans DeviaMain ou inserer ces redefinitions
# Apres l'useEffect localStorage du theme

old_persistance = '''  // Persistance du theme
  useEffect(() => {
    try {
      localStorage.setItem("devia_theme", themeMode);
    } catch (e) {}
...
new_with_styles = '''  // Persistance du theme
  useEffect(() => {
    try {
      localStorage.setItem("devia_theme", themeMode);
    } catch (e) {}
  ...
  // Styles dynamiques selon le theme (shadow les const globaux)
  const t = themes[themeMode] || themes.dark;
  const cardStyle = {
    background: t.cardBg,
...
if "// Styles dynamiques selon le theme" in content:
    print("[INFO] Styles dynamiques deja en place")
elif old_persistance in content:
    content = content.replace(old_persistance, new_with_styles, 1)
    print("[OK] cardStyle/inputStyle/btnPrimary/btnSecondary rendus dynamiques")
    modifs += 1
else:
    print("[ERREUR] Persistance non trouvee")
    sys.exit(1)

# ================================================================
# MOD 4 : Adapter le DIV racine (fond)
# ================================================================
...
if "background: t.bgRoot" in content:
    ...
    print("[OK] DIV racine adapte au theme")
    modifs += 1
else:
    print("[WARN] DIV racine non trouve")

# ================================================================
# MOD 5 : Adapter le header
# ================================================================
...
if "background: t.headerBg" in content:
    ...
    print("[OK] Header adapte au theme")
    modifs += 1
else:
    print("[WARN] Header non trouve")

# ================================================================
# MOD 6 : Adapter la nav (les tabs)
# ================================================================
...
if "background: t.navBg" in content:
    ...
    print("[OK] Nav adaptee au theme")
    modifs += 1
else:
    print("[WARN] Nav non trouvee")

# ================================================================
# MOD 7 : Adapter les boutons de tabs dans la nav
# ================================================================
...
if "background: activeTab === tab.id ? t.navTabActive" in content:
    ...
    print("[OK] Tabs adaptees au theme")
    modifs += 1
else:
    print("[WARN] Tabs non trouvees")

# ================================================================
# MOD 8 : Ajouter le toggle Theme dans Parametres
# On insere avant la section "Mon entreprise"
# Vu qu'on connait pas le marqueur exact, on cherche le debut de la page parametres
# ================================================================

# On vise le titre 'Parametres' qui ouvre la page
# Ou on peut chercher le marqueur '{activeTab === "parametres" &&'

old_parametres_marker = '{activeTab === "parametres" && ('

# On veut inserer une section toggle juste apres l'ouverture
# Mais on a besoin du contexte pour pas casser
# Strategie : inserer apres le <h2> Parametres si trouvable

# Plutot que de se prendre la tete, on cherche un marqueur stable juste apres le h2
# Si tu te souviens, dans la page Compte le h2 etait suivi par un div sous-titre
# Pour parametres c'est sans doute pareil

# Mais on ne sait pas exactement, donc approche alternative :
# On insere juste apres le marqueur d'ouverture de la condition
...
if "Toggle Theme Sombre/Clair" in content:
    ...
    print("[OK] Toggle Theme Sombre/Clair ajoute dans Parametres")
    modifs += 1
else:
    print("[WARN] Marqueur 'activeTab parametres' non trouve - toggle non ajoute")
    print("[INFO] Tu pourras toujours toggler via console : localStorage.setItem('devia_theme','light')")
...
print("  1. State themeMode + persistance localStorage")
print("  2. Objet themes avec 2 palettes (dark + light)")
print("  3. cardStyle/inputStyle/btnPrimary/btnSecondary dynamiques")
print("  4. Fond global adaptatif")
print("  5. Header adaptatif")
print("  6. Nav adaptative")
print("  7. Tabs adaptatives")
print("  8. Toggle Sombre/Clair dans Parametres (en haut de la page)")
print()
print("PROCHAINE ETAPE :")
print("  npm run build")
print()
print("SI BUILD OK :")
print("  - Va sur Parametres")
print("  - Tu vois le nouveau toggle Apparence")
print("  - Click 'Clair' -> bascule immediate !")
print("  - Tu pourras tester si l'aspect te plait avant qu'on continue")
print()
