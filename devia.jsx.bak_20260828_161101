...
const EC5_SECTIONS = [
  // Sections SCIERIE FRANCAISE - gamme continue (sans trous, toutes commerciales)
  // Chevrons / petites pieces
  [60,80],[80,80],[60,100],[80,100],[63,150],[75,150],
  // Intermediaires (le trou 100->200 comble)
  [63,175],[75,175],[63,200],[75,200],
  // Pannes / madriers
  [80,200],[100,200],[80,220],[100,220],[75,225],[100,225],
  // Fortes pannes
  [100,240],[120,240],[100,250],[120,260],
  // Arbaletriers / grosses portees
  [140,280],[150,300],[160,320],[200,300],
...
setPiece("Chevron");
    ...
    const addChevronOnglet = (x, signZ) => {
      ...
      const zE = lg/2 + debZ;                       // egout (aplomb du mur + depasse, coupe au droit d'une aile)
...
setPiece("Empannon de croupe");
  ...
  const drawPenetration = () => {
    ...
    setPiece("Panne faitiere");
    addBox(dR + 0.25, 0.24, 0.12, xw + s * (dR + 0.25) / 2, hF - 0.12, 0, woodMat);
...
const fileInputRef = useRef(null);
  ...
  // VERIFICATEUR DETERMINISTE : rejoue le placement en 2D et controle la coherence.
  ...
  const analyserFichiers = async (fileList) => {
    ...
    try {
      ...
      const sysAnalyse = "Tu analyses des plans/photos de projets de charpente bois. Reponds UNIQUEMENT avec un objet JSON, sans markdown : " +
        ...
        "REGLE DEPASSE DE TOITURE (tres important) : cherche pour CHAQUE volume son depasse de toiture (debord du toit au-dela des murs), note sur le plan de toitures ou les coupes, et recopie-le en CENTIMETRES dans debord_toiture_cm (une cote 0.80 m donne 80, une cote 120 donne 120). Mets null si non lisible, n'invente jamais. GARDE-FOU CONTOUR : sur un plan de toitures le contour dessine INCLUT les depasses (contour = murs plus 2 fois le depasse) ; les cotes d'emprise des murs (longueur, largeur) viennent des plans d'etage ou du plan de masse - ne recopie jamais une cote de contour de toiture comme cote de murs. " +
      ...
      const versionPrompt = vh.toString(36) + "-p3v8";
      let jCache = null;
      ...
      if (decomp.length >= 2) {
        const structs = decomp.map(o => {
          ...
          return {
            ...
            debord: (typeof o.debord_toiture_cm === "number" && o.debord_toiture_cm > 0) ? Math.round(o.debord_toiture_cm) / 100 : undefined,
        ...
        setAnalyseResume(structs.map((s, k) => "V" + (k + 1) + " " + (LT_DECOMP[s.type] || s.type) + " " + s.longueur + "x" + s.largeur + "m" + (s.hauteur ? " h" + s.hauteur + "m" : " h?") + (s.debord ? " depasse " + Math.round(s.debord * 100) + "cm" : "") + (s.pos ? " - contre V" + s.pos.contre + ", " + (s.pos.facade ? "FACADE " + s.pos.facade + (s.pos.alignement ? " aligne " + s.pos.alignement : "") + ", " : "") + String(s.pos.cote).replace("_", " ") + ", faitage " + (s.pos.faitage || "?") + ", decalage " + (s.pos.decalage || 0) + "m" : "")).join(" | "));
      ...
      <div>
        ...
        <div style={cardStyle}>
          ...
          {(() => {
            ...
            // ---- Donnees et mini-graphiques SVG ----
            ...
            const labelsMois = clesMois.map((c) => nomMois[c.m]);
            ...
            const graphBarres = (valeurs, labels, couleur, formatV) => {
              const W2 = 720, H2 = 200, pB = 22, pT = 18, pL = 8, pR = 8;
              ...
              return (
                <svg viewBox={"0 0 " + W2 + " " + H2} style={{ width: "100%", height: "auto", display: "block" }}>
                  ...
                  {labels.map((l, i) => (l ? <text key={"t" + i} x={pL + i * zw + zw / 2} y={H2 - 5} fill="#545870" fontSize="11" textAnchor="middle">{l}</text> : null))}
                  <text x={W2 - pR} y={pT - 3} fill={couleur} fontSize="12" fontWeight="600" textAnchor="end">{"max " + formatV(vMax)}</text>
                </svg>
              );
            };

            const graphLigne = (valeurs, labels, couleur, formatV) => {
              const W2 = 720, H2 = 200, pB = 22, pT = 18, pL = 8, pR = 56;
              ...
              return (
                <svg viewBox={"0 0 " + W2 + " " + H2} style={{ width: "100%", height: "auto", display: "block" }}>
                  ...
                  <text x={W2 - 2} y={Math.max(pT, py(dern) - 6)} fill={couleur} fontSize="12" fontWeight="600" textAnchor="end">{formatV(dern)}</text>
                  {labels.map((l, i) => (l ? <text key={"t" + i} x={px(i)} y={H2 - 5} fill="#545870" fontSize="11" textAnchor="middle">{l}</text> : null))}
                </svg>
              );
            };

            // ---- Donnees categorielles (communes, types, couvertures, montants) ----
